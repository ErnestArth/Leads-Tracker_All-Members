...
// create a team member
  ...
  getClientsUnderUser(userId: string, name: string, status: string,duration: string,page: number , limit: number ) :Observable<getAllClients> {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders({ Authorization: `Bearer ${token}` });
    return this.http.get<getAllClients>(
      `${this.apiUrl}/leads-tracker/api/v1/clients/all-clients/${userId}`,
      { headers, params:{name, status,duration,page,limit} }
    );
  }
